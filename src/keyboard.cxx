...
keyboard::keyboard()
{
   ...
   if(baseFile.is_open()) {
      while(baseFile.good()) {
         ...
         if(!line.compare("layout")) {
            type = LAYOUT;
            continue;
         }
         if(!line.compare("variant")) {
            type = VARIANT;
            continue;
         }
         if(!line.compare("option")) {
            type = OPTION;
            continue;
         }
...
bool keyboard::respondToEvent()
{
   while(1) {
      dialog->wait();
      if(activateMode->getElement()==dialog->eventWidget()) {
         delete dialog;
...
bool keyboard::expertWriteConf()
{
   ...
   for(it=lvg.begin(); it!=lvg.end(); it++) {
      ...
      l.append(layout[it->first]);
      l.push_back(',');
      v.append(variant[it->second]);
      v.push_back(',');
   }
   l.erase(l.size()-1);
   v.erase(v.size()-1);
